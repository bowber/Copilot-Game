...
export const LoginScreen: Component<GameScreenProps> = props => {
  const handleContinue = () => {
    if (props.gameInstance) {
      props.gameInstance.set_player_name('Player');
      props.gameInstance.transition_to_screen('ServerSelection');
    }
  };

  return (
    <div class="game-screen login-screen">
      <div class="screen-content">
        <h1 class="game-title">🎮 RPG Game</h1>
        <p class="subtitle">Welcome to your adventure!</p>
        ...
        <div class="instructions">
          <p>Click "Enter Game" or press Enter to start your adventure!</p>
        </div>
...
export const ServerSelectionScreen: Component<GameScreenProps> = props => {
  const selectRegion = (region: string) => {
    if (props.gameInstance) {
      props.gameInstance.set_region(region);
      props.gameInstance.transition_to_screen('MainMenu');
    }
  };

  return (
    <div class="game-screen server-selection-screen">
      <div class="screen-content">
        ...
        <div class="region-selection">
          <div class="region-info">
            ...
            <Show when={!props.gameState?.region}>
              <p>Choose your preferred server region below</p>
            </Show>
          </div>

          <div class="region-options">
            <button 
              class={`region-btn ${props.gameState?.region === 'EU' ? 'selected' : ''}`}
              onClick={() => selectRegion('EU')}
            >
              🇪🇺 Europe
              <span class="region-desc">Low latency for European players</span>
            </button>
            
            <button 
              class={`region-btn ${props.gameState?.region === 'Asia' ? 'selected' : ''}`}
              onClick={() => selectRegion('Asia')}
            >
              🌏 Asia Pacific
              <span class="region-desc">Optimized for Asian regions</span>
            </button>
            
            <button 
              class={`region-btn ${props.gameState?.region === 'Vietnam' ? 'selected' : ''}`}
              onClick={() => selectRegion('Vietnam')}
            >
              🇻🇳 Vietnam
              <span class="region-desc">Dedicated Vietnamese servers</span>
            </button>
          </div>
          ...
          <div class="region-actions">
            <button 
              class="back-btn"
              onClick={() => props.gameInstance?.transition_to_screen('LoginScreen')}
            >
              ← Back
            </button>
          </div>
...
export const MainMenuScreen: Component<GameScreenProps> = props => {
  const startGame = () => {
    if (props.gameInstance) {
  ...
  return (
    <div class="game-screen main-menu-screen">
      <div class="screen-content">
        <h2>⚔️ Main Menu</h2>
        ...
        <div class="player-info">
          <Show when={props.gameState?.player_name}>
            <p class="welcome-message">
              Welcome back, <strong>{props.gameState!.player_name}</strong>!
            </p>
          </Show>

          <Show when={props.gameState?.region}>
            <p class="region-info">
              🌍 Region: <span class="region-name">{props.gameState!.region}</span>
            </p>
          </Show>
        </div>
        ...
        <div class="menu-navigation">
          <button 
            class="back-btn"
            onClick={() => props.gameInstance?.transition_to_screen('ServerSelection')}
          >
            ← Change Region
          </button>
        </div>
